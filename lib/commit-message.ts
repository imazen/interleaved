...
const defaultCommitTemplates: Record<CommitAction, string> = {
  create: "Create {path} (via Interleaved)",
  update: "Update {path} (via Interleaved)",
  delete: "Delete {path} (via Interleaved)",
  rename: "Rename {oldPath} to {newPath} (via Interleaved)",
};
